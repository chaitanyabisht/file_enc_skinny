...
class SkinnyCipher:
    ...
    def ECB_ENCRYPT(self, key, plaintext):
        # Pad Plaintext
        plaintext = pad(plaintext.encode(), 16, style='pkcs7').hex()
        # Divide plaintext to 128-bit blocks
        blocks = [plaintext[i:i+32] for i in range(0, len(plaintext), 32)]
        # Encrypt each block
        ciphertext = ''
        for block in blocks:
            ciphertext += hex(SkinnyCipherBlock(key).encrypt(int(block, 16)))[2:].zfill(32)
        return ciphertext
    
    def ECB_DECRYPT(self, key, ciphertext):
        # Divide ciphertext to 128-bit blocks
        blocks = [ciphertext[i:i+32] for i in range(0, len(ciphertext), 32)]
        # Decrypt each block
        plaintext = ''
        for block in blocks:
            plaintext += hex(SkinnyCipherBlock(key).decrypt(int(block, 16)))[2:].zfill(32)
        # Remove padding
        plaintext = unpad(bytes.fromhex(plaintext), 16, style='pkcs7').decode()
        return plaintext
    
    def CBC_ENCRYPT(self, key, plaintext, iv):
        # Pad Plaintext
        plaintext = pad(plaintext.encode(), 16, style='pkcs7').hex()
        # Divide plaintext to 128-bit blocks
        blocks = [plaintext[i:i+32] for i in range(0, len(plaintext), 32)]
        ...
        xor_vec = iv
        for block in blocks:
            ciphertext += hex(SkinnyCipherBlock(key).encrypt((xor_vec ^ int(block, 16))))[2:].zfill(32)
            xor_vec = int(ciphertext, 16)
        return ciphertext

    def CBC_DECRYPT(self, key, ciphertext, iv):
        # Divide ciphertext to 128-bit blocks
        blocks = [ciphertext[i:i+32] for i in range(0, len(ciphertext), 32)]
        # Decrypt each block
        plaintext = ''
        xor_vec = iv
        for block in blocks:
            plaintext += hex(SkinnyCipherBlock(key).decrypt(int(block, 16)) ^ xor_vec)[2:].zfill(32)
            xor_vec = int(block, 16)
        # Remove padding
        plaintext = unpad(bytes.fromhex(plaintext), 16, style='pkcs7').decode()
        return plaintext
    
    def CTR_ENCRYPT(self, key, plaintext, nonce):
        # Pad Plaintext
        plaintext = pad(plaintext.encode(), 16, style='pkcs7').hex()
        # Divide plaintext to 128-bit blocks
        blocks = [plaintext[i:i+32] for i in range(0, len(plaintext), 32)]
        ...
        ciphertext = ''
        counter = 0
        vector = int(str(nonce) + str(counter))
        for block in blocks:
            vector_enc = SkinnyCipherBlock(key).encrypt(vector)
            ciphertext += hex((int(block, 16) ^ vector_enc))[2:].zfill(32)
            counter += 1
            vector = int(str(nonce) + str(counter))
        return ciphertext

    def CTR_DECRYPT(self, key, ciphertext, nonce):
        # Divide ciphertext to 128-bit blocks
        blocks = [ciphertext[i:i+32] for i in range(0, len(ciphertext), 32)]
        # Decrypt each block
        plaintext = ''
        ...
        for block in blocks:
            vector_enc = SkinnyCipherBlock(key).encrypt(vector)
            plaintext += hex((int(block, 16) ^ vector_enc))[2:].zfill(32)
            counter += 1
            vector = int(str(nonce) + str(counter))
        # Remove padding
        plaintext = unpad(bytes.fromhex(plaintext), 16, style='pkcs7').decode()
    ...
    def getKey(self, password):
        salt = bytes.fromhex('A1B2C3D4E5F6A1B2C3D4E5F6A1B2C3D4')
        key = PBKDF2(password, salt, dkLen=16, count=100000, hmac_hash_module=SHA512)
...
if __name__ == "__main__":
    ...
    cipher = SkinnyCipher()
    message = 'A'*16 + 'B'*16 + 'C'*16 + 'D'*16 + 'E'
    # encode message to hex
    message_hex = message.encode('utf-8').hex()
    # ECB
    print('ECB')
    ct = cipher.ECB_ENCRYPT(0x17401096d712b2adcc0143a91dddb11c, message_hex)
    pt = cipher.ECB_DECRYPT(0x17401096d712b2adcc0143a91dddb11c, ct)

    # decode hex to string
    output = bytes.fromhex(pt).decode('utf-8')
    if (output != message):
        print('Error ECB')
    ...
    print('CBC')
    # 0xff401096d71ff2adccff43a91ffdb11c
    ct = cipher.CBC_ENCRYPT(0x17401096d712b2adcc0143a91dddb11c, message_hex, 0xff401096d71ff2adccff43a91ffdb11c)
    pt = cipher.CBC_DECRYPT(0x17401096d712b2adcc0143a91dddb11c, ct, 0xff401096d71ff2adccff43a91ffdb11c)
    ...
    ct = cipher.CTR_ENCRYPT(0x17401096d712b2adcc0143a91dddb11c, message_hex, 0xff401096d71ff2adccff43a91ffdb1)
    pt = cipher.CTR_DECRYPT(0x17401096d712b2adcc0143a91dddb11c, ct, 0xff401096d71ff2adccff43a91ffdb1)
